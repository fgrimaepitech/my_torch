...
class ConvTranspose2d(Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        up_h = (height - 1) * sh + 1
        up_w = (width - 1) * sw + 1
        upsampled = xp.zeros((batch_size, in_channels, up_h, up_w), dtype=x.data.dtype)
        ...
        flipped_weight = xp.flip(self.weight.data, axis=(2, 3))
        conv_h = padded.shape[2] - kh + 1
        conv_w = padded.shape[3] - kw + 1
        output = xp.zeros((batch_size, self.out_channels, conv_h, conv_w), dtype=x.data.dtype)

        for b in range(batch_size):
            for oc in range(self.out_channels):
                channel_sum = xp.zeros((conv_h, conv_w), dtype=x.data.dtype)
                for ic in range(in_channels):
                    k = flipped_weight[ic, oc]
                    for i in range(conv_h):
                        for j in range(conv_w):
                            patch = padded[b, ic, i:i+kh, j:j+kw]  # always (kh, kw)
                            channel_sum[i, j] += xp.sum(patch * k)
                output[b, oc] = channel_sum

        # 4) Apply output_padding by padding on the bottom/right (PyTorch semantics)
        if oph or opw:
            output = xp.pad(output, ((0, 0), (0, 0), (0, oph), (0, opw)), mode='constant')
