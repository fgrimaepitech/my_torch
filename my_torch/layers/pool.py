...
class MaxPool2d(Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # Calculate output dimensions (PyTorch formula)
            ...
            for c in range(channels):
                for i in range(out_height):
                    for j in range(out_width):
                        ...
                        # Extract window (handle edge cases)
    ...
    def _pad2d(self, x: Tensor, padding: tuple) -> Tensor:
        ...
        if ph == 0 and pw == 0:
            return x
        # Use a very negative value for padding in MaxPool2d (PyTorch behavior)
        # This ensures padded values are never selected by max pooling
